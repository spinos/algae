...
ParsePCache::ParsePCache() {}

ParsePCache::~ParsePCache() {}

MSyntax ParsePCache::newSyntax() 
{
	MSyntax syntax;
...
MStatus ParsePCache::doIt( const MArgList& args ) 
{
	getTmpPath();
...
// get cache file	
				...
				int num_pass = fviz.findPlug("numPass").asInt();
				float constant_width = fviz.findPlug("constantWidth").asFloat();
...
#ifdef WIN32
						sproc = MString("Procedural \\\"DynamicLoad\\\" [\\\"zFluidParticleProcedural.dll\\\" \\\"") +argbuf+"\\\"] "+bboxbuf;
#else
						sproc = MString("Procedural \\\"DynamicLoad\\\" [\\\"zFluidParticleProcedural.so\\\" \\\"") +argbuf+"\\\"] "+bboxbuf;
#endif
...
void* ParsePCache::creator() {
 return new ParsePCache;
 }
 
MStatus ParsePCache::parseArgs( const MArgList& args )
{
	// Parse the arguments.
...
MObject ParsePCache::getDirectEnsembleNode(MPlug& plg, MString& objname, MString& passname)
{
	MStatus status;
		MObject oconn;
...
void ParsePCache::defTraceBias(float val)
{
...
void ParsePCache::defDisplaceBound(float val)
{
...
void ParsePCache::injectRIBStatement(MObject& node)
{
...
void ParsePCache::injectShaderStatement(MObject& node, MString& objname, MString& passname, int type)
{
	if(node == MObject::kNullObj) return;
// shader and scene path
	MString proj;
...
MString ParsePCache::funcOrVarNode(MObject& node, MString& objname, MString& passname, VariableList& downstreamargs)
{
	MStatus status;
	MString res;
	MFnDependencyNode fnode(node);
...
int ParsePCache::getMatchedCondition(MObject& node, MString& name)
{
	MFnDependencyNode fnode(node);
	int nsit = fnode.findPlug("numCondition").asInt();
...
MString ParsePCache::valueVarNode(MObject& node)
{
	MFnDependencyNode fnode(node);
	char valbuf[256];
...
void ParsePCache::valueFromPieceNode(int type, const char* nmattr, MPlug& pgattr, MObject& node, SLVariable* var)
{
	MFnDependencyNode fnode(node);
	float x, y, z;
	char valbuf[256];
...
void ParsePCache::valueFromVarNode(MObject& node, SLVariable* var, int& access)
{
	MFnDependencyNode fnode(node);
	var->name = fnode.name().asChar();
...
void ParsePCache::convertType(int type, SLVariable* var)
{
	switch(type) {
		case RSLFloat:
			var->type = "float";
...
void ParsePCache::pathExpression(string& res, MString& objname)
{
	MString proj;
	MGlobal::executeCommand( MString ("string $p = `workspace -q -fn`"), proj );
	MString scn;
